...
local function get_all_unit_groups()
  ...
  for each, surface in pairs(game.surfaces) do
    ...
    for every, biter in pairs(biters) do
      if biter.unit_group then
        if not global.unit_groups[biter.unit_group.group_number] then
          global.unit_groups[biter.unit_group.group_number] = {
            group = biter.unit_group,
            position = biter.unit_group.position
          }
        end
      end
    end
...
script.on_event(defines.events.on_unit_group_created, function(event)
  if not global.unit_groups then
    global.unit_groups = {}
    global.unit_groups[event.group.group_number] = {
      group = event.group,
      position = event.group.position
    }
  else
    global.unit_groups[event.group.group_number] = {
      group = event.group,
        ...
        if not group.valid then
          ...
          -- if biter.active then
            ...
            -- if biter.moving then
              ...
              -- end
              local color = make_rainbow(event_tick, group_number, frequency, palette_choice)
              -- local color = {.5,.5,.5}
              -- local last_position = data.position
            ...
            global.unit_groups[group_number].position = current_position
